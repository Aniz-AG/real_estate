...
const Toast = ({ message, type, onClose }) => (
  <motion.div
    ...
    className={`fixed bottom-6 right-6 z-[99999] flex items-center gap-3 px-6 py-4 rounded-xl shadow-2xl backdrop-blur-sm ${
      type === "success"
...
const formatSearchSummary = (summary) => {
  if (!summary) return "No activity yet";
  const topCity = summary.top_city || "No city";
  const topType = summary.top_type
    ? summary.top_type.replace(/_/g, " ")
    : "No type";
  return `${topCity} · ${topType}`;
};

const formatMainSearch = (mainSearch) => {
  if (!mainSearch) return "No searches yet";
  const parts = [
    mainSearch.city,
    mainSearch.property_type?.replace(/_/g, " "),
    mainSearch.bhk_type,
  ].filter(Boolean);
  const budget =
    mainSearch.min_price || mainSearch.max_price
      ? `Budget: ${mainSearch.min_price || "Any"} - ${mainSearch.max_price || "Any"}`
      : null;
  return [parts.join(" • "), budget].filter(Boolean).join(" | ");
};
  ...
  const fetchUsers = async () => {
    try {
      ...
      const res = await fetch(
        `/api/admin/users?page=${page}&limit=12&search=${debouncedSearch}`,
        {
          credentials: "include",
        },
      );
                ...
                >
                  <Card className="border-0 shadow-xl bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm overflow-hidden group">
                    ...
                    <CardContent className="p-6">
                      ...
                      <div className="border border-gray-100 dark:border-gray-700 rounded-xl p-4 bg-white/70 dark:bg-gray-900/40 mb-4">
                        ...
                        <div className="space-y-2 text-sm text-muted-foreground">
                          <div className="flex items-center gap-2">
                            ...
                            <span>
                              Last seen:{" "}
                              {formatDateTime(u.activity_summary?.last_seen)}
                            </span>
                          </div>
                          <div className="flex items-center gap-2">
                            ...
                            <span>
                              Interest:{" "}
                              {formatSearchSummary(u.activity_summary)}
                            </span>
                          </div>
                          <div className="flex items-center gap-2">
                            ...
                            <span>
                              Main search:{" "}
                              {formatMainSearch(
                                u.activity_summary?.main_search,
                              )}
                            </span>
                          </div>
                          <div className="flex items-center gap-4 text-xs text-gray-500">
                            <span>
                              {u.activity_summary?.search_count || 0} searches
                            </span>
                            <span>
                              {u.activity_summary?.like_count || 0} likes
                            </span>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="mb-4">
                        ...
                        {u.likes && u.likes.length > 0 ? (
                          <div className="space-y-2">
                            {u.likes.slice(0, 3).map((property) => (
                              <Link
                                key={property._id}
                                href={`/property/${property._id}`}
                                className="flex items-center gap-3 rounded-lg border border-gray-100 dark:border-gray-700 p-2 hover:bg-gray-50 dark:hover:bg-gray-800/40 transition-colors"
                              >
                                <img
                                  src={
