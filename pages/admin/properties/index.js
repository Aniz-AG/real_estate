...
const Toast = ({ message, type, onClose }) => (
  <motion.div
    ...
    className={`fixed bottom-4 right-4 z-[99999] flex items-center gap-3 px-5 py-3 rounded-xl shadow-2xl backdrop-blur-sm ${
      type === "success"
...
export default function ManageProperties() {
  ...
  const fetchProperties = async () => {
    ...
    try {
      const res = await axios.get(
        `/api/admin/properties?page=${page}&search=${searchQuery}`,
      );
  ...
  const handleToggleFeature = async (propertyId, featureType, currentValue) => {
    setTogglingFeature(`${propertyId}-${featureType}`);
    try {
      const res = await axios.put("/api/admin/properties/toggle-feature", {
        propertyId,
        featureType,
        value: !currentValue,
      });
      ...
      setProperties((prev) =>
        prev.map((p) =>
          p._id === propertyId ? { ...p, ...res.data.property } : p,
        ),
      );

      const featureName = featureType.replace("is_", "").replace("_", " ");
      showToast(
        `Property ${!currentValue ? "added to" : "removed from"} ${featureName}`,
      );
    } catch (error) {
                      ...
                      <div className="relative h-48 overflow-hidden">
                        ...
                        <div className="absolute top-3 right-3 flex flex-wrap gap-1">
                          {property.is_featured && (
                            <span className="px-2 py-0.5 rounded-full text-xs font-semibold bg-amber-500 text-white flex items-center gap-1">
                              <Star className="h-3 w-3" /> Featured
                            </span>
                          )}
                          {property.is_top_project && (
                          ...
                          <div className="flex flex-wrap gap-2">
                            ...
                            >
                              ...
                              `${property._id}-is_featured` ? (
                                ...
                                <Star
                                  className={`h-3.5 w-3.5 ${property.is_featured ? "fill-current" : ""}`}
                                />
                              )}
                              Featured
                            </button>
                            ...
                            <button
                              ...
                              disabled={
                                togglingFeature === `${property._id}-is_premium`
                              }
                              className={`flex items-center gap-1 px-2.5 py-1.5 rounded-lg text-xs font-medium transition-all ${
                                property.is_premium
